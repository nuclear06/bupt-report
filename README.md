...
## 本程序有待debug！！！03/30
### 欢迎帮我debug,pr就完事辣
本程序会在北京时间`01:30`,`13:30`, `19:30`进行签到，并通过邮件返回是否打卡成功的信息,默认情况下打卡成功会返回填报的具体信息，失败会返回日志，对于前者可以修改`parameter.py`中的`RIGHT_RETURN = True`为`RIGHT_RETURN = False`，此时打卡成功仍会返回邮件，但是不会返回具体信息
\
